"""
The base classes for classification models.
"""
...
from ..utils.metrics import cal_binary_classification_metrics
import pickle
    ...
    def _train_model(
        ...
        try:
            ...
            for epoch in range(self.epochs):
                ...
                if self.measure_metric == 'f1' and np.not_equal(self.best_loss, float("inf")):
                    predictions = torch.cat(prediction_collector)
                    predictions = predictions.cpu().detach().numpy()        
                    val_labels = torch.cat(val_label_collector)
                    val_labels = val_labels.cpu().detach().numpy()
                  
                    #logger.info(f"predictions is {predictions.shape}  val_labels is {val_labels.shape}")
                    metrics = cal_binary_classification_metrics(predictions, val_labels)
                    current_f1 = metrics["f1"]
                    logger.info(f"The best f1 is {self.best_f1}  Current f1 is {current_f1}") 
